...
function updateFinalState(){
    ...
    if(solved.length >= tests.length){
        ...
        finalText.innerHTML = "Has conseguido todas las llaves. Ya puedes abrir el regalo final.";
    } else {
        finalBox.classList.add("locked");
        finalText.innerHTML = `Necesitas ${tests.length - solved.length} llave(s) más para desbloquear la sorpresa.`;
    }
}
